...
public class CollectionsFragment extends
    ...
    private final int LIMIT = CommonConstants.NormalCons.LIMIT_10;
    ...
    @BindView(R.id.view_search_collections)
    InputEdit mSearchEdit;
    ...
    @Override
    public CollectionsContract.ICollectionsPresenter initPresenter() {
    ...
    @Override
    public void showCollect(List<CollectedRepo> repoList) {
    ...
    @Override
    public void showErrorOnQueryFailure(String error) {
        toast(error);
        if (!mCollectAdapter.isLoadingMore()) {
            switchStatus(Status.STATUS_NORMAL);
            UiUtils.stopRefresh(mRefreshLayout);
    ...
    @Override
    public void showCollectQueryByKey(List<CollectedRepo> repoList) {
        showCollect(repoList);
    }

    @Override
    public void showQueryByKeyFailure(String error) {
        showErrorOnQueryFailure(error);
    }

    @Override
    public void showCollectionCanceled(CollectedRepo repo) {
        toast(R.string.tip_collect_cancel_success);
        mCollectAdapter.remove(repo);
    }

    @Override
    public void showCollectionCancelFailure() {
    ...
    public void onItemClick(View view, int position) {
        ...
        Actions.openUrl(this.getActivity(), repo.getHtmlUrl());
    }
